...
def load_internvl_model():
    ...
    device = "cuda" if torch.cuda.is_available() else "cpu"  # Use CUDA
    ...
    try:
        model = AutoModel.from_pretrained(
            path,
            torch_dtype=torch.float16, # float16 is generally faster on RTX GPUs
            quantization_config=quantization_config,
            low_cpu_mem_usage=True,
            use_flash_attn=True,  # Enable FlashAttention (if installed)
...
def extract_text_from_image(image_file, model, tokenizer, device):
    ...
    pixel_values = load_image(image_file, max_num=12).to(torch.float16).to(device)  # Use float16 and move to CUDA
...
"""
    ...
    if hasattr(model, 'batch_chat'):
        responses = model.batch_chat(tokenizer, None, prompts=prompts, generation_config=dict(max_new_tokens=512, do_sample=False))
